...
# Jena ARQ directory.
export ARQROOT=~/CHAIN/code/jena/apache-jena-2.12.1
...
# Example based on EUstatistics to run on SEPA datasets(?)
...
# query1
# Example, derived as a variation of SEPA data, run over the SEPA datasets
...
# SPA
# Example query derived from the SPA dataset (which we don't have)
...
# (uses demo-dataset/water.n3 as a source)
...
export DAISY_OUTPUT="${DAISY_HOME}/output"
...
# SWI-Prolog version
# pl -s callProcesses.pl # Out of date HW SWI-Prolog
swipl -s callProcesses.pl 
