:- module(callProcesses, [respond_to_query/0]).
...
:- use_module('translation.pl').
:- use_module('handleDatasetNames.pl').
:- use_module('wordnetSearch.pl').
:- use_module('sumoSearch.pl').
:- use_module('spsmMatch.pl').
:- use_module('narrowDatasets.pl').
:- use_module('repair.pl').
...
respond_to_query(TargetFilesType) :-
    new_output,
    write_heading(['Respond to query: try initial query on target dataset']),
    query_path('query.pl', QueryFile),
    open(QueryFile, read, QF),
    read(QF, QueryTerm),
    close(QF),
    write('Query: '), writeq(QueryTerm), nl,
