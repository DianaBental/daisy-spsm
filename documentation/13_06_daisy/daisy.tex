...
\author{Fiona McNeill, Andriana Gkaniatsou, Alan Bundy }
\vspace{-10pt}
...
\begin{small}
\indent\textbf{SELECT} $reporter\_ID, node, level, date$\\
\indent{\textbf{FROM} $measurement$}\\
\end{small}
...
\indent $reading(Node,Date,Water\_level)$\\
...
\marginpar{moved it to future work}
\marginpar{I don't know whether this is future work or just a possible scenario}
CHAIn can operate completely automatically because the matching process ranks potential matches.  It is, therefore, possible  choose the best match at each stage, to produce a single best-ranked response to the original query.  Setting CHAIn to be fully automated is the best approach where responses are needed very quickly and quality of matching is not important.  However, we also view CHAIn as a method for allowing human users to interact with large datasources quickly and effectively, providing them with the tools to make informed decisions.  It is therefore the general expectation that when the matching process is completed, a set of results passing a given threshold for matching quality, or constrained by the number of required results, is presented to a user that owns, or belongs to an organisation that owns, the queried datasource.  \cbstart These results will be presented with clear information about where approximations were made, where parts of the query were left unanswered, and so on. \marginpar{added example of the current output. review-3 - but not sure if this is what he meant } For example:\\
Original Query : water(resource, measure, geo, timePeriod)\\
Suggested Schema: waterBodyPressures(identifiedDate, waterBodyId, assessmentCategory, source).\\
Schema Mappings:\\
water $\mapsto$ waterBodyPressures\\
resource  $\mapsto$source\\
measure $\mapsto$ assessmentCategory\\
geo $\mapsto$ waterBodyId\\
timePeriod  $\mapsto$ identifiedDate\\
\cbend  This allows for the input of local knowledge into context-free matching, and helps datasources where localised terminology is used to become more universal comprehensible.  For example, $reading$ is considered by WordNet to be a direct hypernym of $measurement$, so a match may be found on that basis.  However, someone who understood the data may know that $reading$ in this context refers to book readings, and is not a correct match for $measurement$.  These kinds of poor matches are likely to be identified through poor matches in the arguments of the terms, but human input can still be valuable.
...
\begin{enumerate}
\item A query to a particular datasource is received by the organisation (or individual) that owns the datasource.
\item If the query fails, it is first determined whether this is due to mismatch at the schema level.  If not, go to step 6.  If so, naive matching is done on the schema of the datasource to narrow down the search space to likely matches (see Section \ref{sec:narrowing}). 
...
\item The query is reformulated according to the matches and resent to the datasource (see Section \ref{sec:reform}).
\item If the query fails, since  at this stage  the query is correct with respect to the schema, we look for mismatches at the data level (see Section \ref{sec:reform}). 
\item Potential responses to the query passing a given threshold are presented, with appropriate annotation describing the match, to a user who is knowledgable about the datasource.  This user then choses any number of matches to be returned to the querier.  (This step can be omitted and the responses can be returned according to automatic ranking if required) (see Section \ref{sec:returning}).
...
\subsection{Format of the datasource}
...
\subsection{\label{sec:narrowing}Narrowing down search space}
  The schema matching part of the process is done by the SPSM algorithm, which does pairwise matching of structured terms.  This is an expensive process, and it is not feasible to do it between a single query and every possible (potentially large) datasource that we own.   It is therefore necessary to perform a filtering step, narrowing down the datasources to a subset of likely candidates.  \cbstart This is done through fairly naive keyword matching the query datasource name to the  datasources we own. This allows us to select a set of  datasources and their corresponding schemata as candidates for matching the query. Throughout all this process we do not alter any of the data\footnote{There is a considerable body of work on matching keywords, and incorporating some of this would allow us to develop a more sophisticated approach to this step of the process.  We have chosen to do this in a naive way because we are developing a proof-of-concept system, but this would be an important part of future work.}.\cbend  \eat{The query predicate name is annotated with synonyms, hyponyms and hypernyms, and the schema of the datasource is sorted to extract parts of it that match any of these terms}  There is, of course, a tension in determining how wide this net should be cast: we do not wish to exclude potentially good matches; however, we do not wish to end up with a very large subset of the data, to perform complex matching on which would be very expensive.  Initial results (see Section \ref{sec:eval})  indicate our approach is workable, but extensive simulation is necessary to determine more precisely where this sweet spot lies.

   After a query failure for the query dataset name, we compute the \textit{Related Terms} set, to which we then try to match the dataset names we own. The schema of each of the matching dataset names and the query schema are the SPSM input. \eat{Then for each dataset name that matches, we extract the corresponding schema which we will send to SPSM.} 
 \textit{Related Terms} is computed based on  the SUMO and Wordnet ontologies, and the process is explained below. 
...
%    \begin{itemize}
%     \item  	Compute all SUMO Related Terms: subClassOf, superClassOf, geographicSubRegion, overlapsSpatially
%	      \item Compute all Wordnet Related Terms; synset, subClass, superClass, hypernym, hyponym, similar, meronym
%
...
{\bf SUMO} Compute \textit{Related Terms}:
For the following process we consider (i) the original query datasource name $Q$, (ii) the set $SQ=\{m_1,m_2,...,m_l\}$ which consists of all subwords of the original query datasource name (if $Q$ is multi-word).
\vspace{-2ex}
\begin{itemize}
\item  $i \in$ \textit{Related Terms} if: 
	\begin{itemize}
	\item $relates(i, Q)$ and/or $relates(Q,i)$ and $relates \in SUMO$ 
	\end{itemize} 
\item $n \in$ \textit{Related Terms} if:
	\begin{itemize}
	\item  $relates(n, m_i)$ or  $relates(m_i, n)$ and $relates \in SUMO$ and $m_i \in SQ$
	\item $relates(n, m_k)$ or  $relates(m_k, n)$ and $relates \in SUMO$ and $m_k \in SQ$
	\item $m_i \neq m_k$ 
	\end{itemize}
\end{itemize}
 ...
 \eat{The first step is to check whether a dataset name consists of multiple subwords\footnote{We have applied the same technique to all SUMO words, \textit{i.e. the SUMO file contains all terms that original appear in SUMO and the same terms splitted into subwords.}}.
\eat{
If so, we split it into individual words.}  

%\vspace{-2ex}
\begin{itemize}
\item Split (if multi-word) the query dataset name into a list individual words. 
\begin{itemize} 
\item For each individual word   compute all SUMO related terms.
\item Compute all SUMO related terms for the usnplit query dataset name.
\eat{\begin{itemize} 
\item  search for all SUMO terms that it may be related to.    
\end{itemize}}
...
%\end{itemize} 
\item Split all SUMO terms into individual words.  
\begin{itemize}
\item For each individual subword that belongs to the query dataset name, search for all SUMO multi-word terms  related  to that subword. 
Then, given the query dataset name compare the SUMO results and keep only the SUMO terms that are related to more than one subwords within the same 
dataset name.
\end{itemize}
\end{itemize}}
\vspace{-1.5ex}
...
{\bf Wordnet}	Compute \textit{Related Terms}:	
\eat{In this step we only consider individual words for each dataset name. Thus, we . 
	The algorithmic steps we follow are:}For the following process we consider the set $SQ=\{m_1,m_2,...,m_l\}$ which consists of all subwords of the original query datasource name (if it is not multi-word, then $SQ$ consists of a single word -the original datasource name).
\vspace{-2ex}
\begin{itemize}
%\item $QS= {m_1,m_2,...,m_l}$ is the set of all subwords the query dataset name consists of. 
\item For each  $m \in QS$ 
			\begin{itemize}
				\item find the Wordnet id of that word
				\item  \textit{Related Terms} is the set of all $Hyponym$, $Hypernym$, $Subclass$, $Superclass$, $Meronym$,   $Similar$ that relate to $m$.
...
{\bf Schema Candidates For Matching}	
 Decompose the datasources names we own into sub-words, if possible. Then, \eat{match the query datasource to the datasources we own by:} \begin{inparaenum}[(i)] \item Match the datasources names with the SUMO and Wordnet \textit{Related Terms} we previously computed, \item Perform string matching. \end{inparaenum} 



 For example, the \textit{Related Terms} for the predicate name  $measurement$ would consist of $38$ instances, including $sampling$, $observation$ and $reading$. We would then target datasource for any predicates with any of these names, and return them.  Perhaps, for example, it would find an exact match for $measurement$ and a match for the \textit{Related  Term} $reading$. 
\eat{this process would find 38 related terms for the predicate name $measurement$, including $sampling$, $observation$ and $reading$.  The narrow-down process would then search the target datasource for any predicates with any of these names, and return them.  For example, it would find an exact match for $measurement$ and a match for the related term $reading$.}

\subsection{\label{sec:spsm} Structure-preserving semantic matching}
\marginpar{Explained why we need spsm} 
\cbstart 
To match the query to any of the datasources that we own, we need to establish whether there exists a correspondence between their structures \textit{i.e.} schemata. Thus, we need full graph-like structure matching, rather than matching the nodes of graph-like structures. The Structure-Preserving Semantic Matching (SPSM) algorithm performs full graph-like structure matching  matching, thus, 
 we consider it as the most appropriate matcher for  our project. \cbend 
 SPSM has been extensively described in ~\cite{spsm}, so we do not cover it in detail here.  \eat{Rather, we provide the reader with sufficient information to understand the principals of the process.}
...
The SPSM process returns a list of ranked responses.  We see from the narrow-down process (see Section \ref{sec:narrowing}) that there are two predicates in the target datasource which are considered relevant: \begin{small} $measurement$ \end{small} and \begin{small}
 $reading$\end{small}. These predicates, together with their arguments, are then sent to SPSM.  In this case, the results are:

\noindent \begin{small}$1.$ $reading(Node,Date,Water\_level)$\\
$2.$ $measurement(Area,Wind\_speed,Direction,Date)$ \end{small}
...
schematically correct. We consider the following cases:
\vspace{-2ex}
\begin{itemize}
 \item The query dataset name can match to more than one dataset name.
\item Each dataset match comes with the corresponding argument matches.
\item Within the same dataset, an argument can have more than one match (more than one argument that can be mapped to).
...
\textbf{Forming the final query - Data Level Repair} The inputs of this process are all query schemata that have been formed by the previous process and for each schema the corresponding data.
...
correct, we  uninstantiate the data values of the query.  The goal of this process is to identify the data values that cause the query failure. 
The steps we perform are the following:
\vspace{-1ex}
\begin{itemize}
 \item Find all data values of the query.
    \begin{itemize}
    \item Select one data value at the time and uninstantiate it.
    	
   
    	
   \item Send new query to the dataset.
    \item Check if it returned any answers.
		\begin{itemize}
		\item If no answers are returned, repeat for all  uninstantiated data combinations systematically until one returns answers.
		% pick next data value, and uninstantiate it. Send the new query to the dataset and check for answers.
	 %	\item Create all uninstantiated data combinations systematically.	
		\end{itemize}
...
\end{itemize}
\vspace{-1.8ex}
A robust evaluation of this approach requires these queries to be generated during the kind of automated process we envisage this work be used in, and the effectiveness of these matches determined through large-scale simulation, to determine what the actual impact of using these matches are.  This is outside the scale of our current pilot project (see Section \ref{sec:concs} for discussions of our plans to do this). We have therefore carried out the evaluation as follows:
\begin{inparaenum}[(i)]
\item We have sourced appropriate data from a collaborator, SEPA (Scottish Environmental Protection Agency).
\item We have sourced online different datasources which discuss similar things, such that we might expect a query from such a datasource to find an appropriate response in our SEPA data.
\end{inparaenum}


For each evaluation test we select a dataset and we formed a query based on the schema of that dataset\footnote{The selected dataset always has relevant information with those to query.}.  Then, we send the query to the remaining datasets we own.  
...
In this section, we present two tests, to give a flavour for the results we have achieved.  In both cases the queries are sent to SEPA datasources\footnote{SEPA datasources: bathingWaterLocations, waterBodyPressures, waterBodyMeasures, surfaceWaterBodies, bathingWaters, classifications.}.\marginpar{I think we should show the datasources schemata, but i haven't due to space limitations}

\textbf{Test 1} We form a query based on the EuroStat\footnote{epp.eurostat.ec.europa.eu} Fresh Water Resources dataset schema: \indent \begin{small} water( resource, measure, geo, timePeriod)
%\footnote{Resource stands for the type of the water body, measure stands for the measurement of the water body, geo stands for the area of the water body and timePeriod stands for the date of that measurement.}
 \end{small} The narrow-down process returned the following dataset names as matching candidates:\\
\begin{small}
 \indent waterBodyPressures\\
 \indent waterBodyMeasures\\
 \indent surfaceWaterBodies\\
 \indent bathingWaterLocations\end{small}\\
SPSM retuned the following matches:\\
\vspace{-4ex}
\begin{itemize}
\item \begin{small}  water $\mapsto$ waterBodyPressures \\
 \indent water(timePeriod) $\mapsto$ waterBodyPressures(identifiedDate)\\
 \indent water(geo) $\mapsto$ waterBodyPressures(waterBodyId)\\
 \indent water(measure) $\mapsto$ waterBodyPressures(assessmentCategory)\\
 \indent water(resource) $\mapsto$ waterBodyPressures(source) \end{small}
\end{itemize}
\vspace{-1.5ex}
\cbstart
\eat{A sample of the answers we retrieved is presented in Table~\ref{waterBodyPressures(identifiedDate,waterBodyId,assessmentCategory,source)}. We do not return exact answers to that query, however, the answers are relevant.

\begin{table}[!h]
\caption{Answers for query waterBodyPressures(water( resource, measure, geo, timePeriod)}		
\begin{center}
\begin{tabular}{ l l c  r }
\hline
identifiedDate & waterBodyId  & assessmentCategory & source \\ \hline
"2008-04-01"   & 23088 & "Morphology and Fish Continuity" & "River" \\
 "2008-04-01"   & 20497 & "Morphology and Fish Continuity" & "River" \\
 "2008-04-01"   & 20788 & "Morphology and Fish Continuity" & "Lake"  \\
 "2008-04-01"   & 23332 & "Morphology and Fish Continuity" & "River" 
\end{tabular}
\label{waterBodyPressures(identifiedDate,waterBodyId,assessmentCategory,source)}
\end{center}
\end{table}}
\marginpar{added 1)resulted query, 2) timings. review-2}
After applying the matches, the reformulated query is:  \begin{small} waterBodyPressures(identi-\\fiedDate, waterBodyId, assessmentCategory, source) \end{small}, which we sent to the datasources and retrieved the corresponding answers. The timings from the first call of our system, until the finish are: 2m10.208s. From which  0m49.725s is the amount of CPU time spent in user-mode code and 0m8.023s is  the amount of CPU time spent in the kernel within the process.
\cbend



\textbf{Test 2} We formed a query based on the UK Environment Agency\footnote{www.environment-agency.gov.uk/}, Bathing Waters dataset schema:\indent \begin{small}  uk$\_$BathingWaters( sampleClassification, prefLabel, long, lat, northing, easting,  latestComplianceAssessment, type, country,\\ district,latestBathingWaterProfile, sedimentTypesPresent, uriSet, regional-\\  Organization, yearDesignated, latestSampleAssessment, eubWidNotation, water-\\  QualityImpacted- ByHeavyRain, zoneOfInfluence, samplingPoint, compliance-\\  Classification, primary- Topic, extendedMetadataVersion, definition, label)\end{small}. The narrow-down process returned the following dataset names as matching candidates:\\ 
\begin{small}
 \indent  bathingWaters\\
 \indent bathingWaterLocations\\
 \indent waterBodyMeasures\\
 \indent waterBodyPressures\\
\end{small}
SPSM retuned the following matches:\\ 
...
\item  \begin{small}
 \indent uk$\_$BathingWaters $\mapsto$ waterBodyMeasures\\
 \indent uk$\_$BathingWaters(label)$\mapsto$ waterBodyMeasures(waterBodyId)\\
 \indent uk$\_$BathingWaters(primaryTopic)$\mapsto$ waterBodyMeasures(secondaryMeasure).\\
\vspace{-1.5ex}
\item 
 \indent uk$\_$BathingWaters $\mapsto$ waterBodyPressures\\
 \indent uk$\_$BathingWaters(label)$\mapsto$  waterBodyPressures(waterBodyId)\\
 \indent uk$\_$BathingWaters(type) $\mapsto$ waterBodyPressures(pressureType)\\
 \indent uk$\_$BathingWaters(prefLabel) $\mapsto$ waterBodyPressures(activity)\\
 \indent uk$\_$BathingWaters(sampleClassification) $\mapsto$  waterBodyPressures(affectsGroundwater)\\
\vspace{-1.5ex}
\item 
 \indent uk$\_$BathingWaters $\mapsto$ bathingWaters.\\
 \indent uk$\_$BathingWaters(label) $\mapsto$ bathingWaters(description)\\
\end{small}
\end{itemize}
\vspace{-4ex}
\cbstart
\marginpar{added 1) resulted queries, 2)timings. review-2}
After applying the above matches we form the following queries
\begin{inparaenum}[(i)]
\item \begin{small}  waterBodyMeasures(waterBodyId, secondaryMeasure) \end{small}
\item \begin{small}   waterBodyPreassures( waterBodyId, pressureType, activity) \end{small}
\item  \begin{small} bathingWaters(description)\end{small}
\end{inparaenum}
 which we send to the datasets and retrieve the corresponding answers. The timings from the first call of our system, until the finish are: 3m55.407s. From which 1m25.673s is the amount of CPU time spent in user-mode code and  0m18.614s is  the amount of CPU time spent in the kernel within the process.
...
The work in this paper is concerned with applying techniques that have already been evaluated and used in other contexts to a new domain.  The SPSM algorithm is novel in that whilst it is built on standard tree-edit-distance techniques (e.g., in \cite{treematching}, using semantic matching to determine what impact on the overall meaning of a tree altering or removing will have.  Previously, it has been used to perform service integration and peer selection in a peer-to-peer network \cite{trust}.  The novelty of our work is in developing a system that can use this structured matching techniques to address the problem of returning useful results to a query on a datasource which has not been correctly formulated according to the schema and data within that datasource.
