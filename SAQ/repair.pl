...
repair_with_schema_maps(QueryTerm, Mappings, NewQueries) :-
    QueryTerm =.. [entity | Params],
    % Use setof to generate all the query mappings at once.
    setof(NewQuery,
	  repair_with_schema_maps_sub(Params, Mappings, NewQuery),
	  NewQueries),
    !.
repair_with_schema_maps(_QueryTerm, _Mappings, []).

% repair_with_schema_maps_sub(+Params, +Mappings, -NewQuery)
% Params = [resource, measure, geo, timePeriod]  (i.e. query columns + values)
% Mappings = mapschema(score(0.142857), 
%                       [dataset(water, waterBodyMeasures), 
%                        column(resource, dataSource), 
%                        column(measure, measureFixedDate),
%                        column(measure, measureId), 
%                        column(timePeriod, waterBodyId)])
% Params = [type(germanCity), inhabitants]  (i.e. query columns + values)
% Mappings = [mapschema(score(0.33), [dataset(germanCity, 'City')])]
repair_with_schema_maps_sub(Params, Mappings, NewQuery) :-
    ...
    member(dataset(_Source, Target), Map),
    ...
    NewQuery =.. [entity, type(Target) | Result].
%    nl, write('New query: '),
%    write(NewQuery).
