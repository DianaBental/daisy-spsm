...
start_translation(TargetFilesType, Query, Success) :-
    nl, write('Construct fully realised (SPARQL) query for '), writeq(Query), nl, nl, 
...
create_one_python_entry(Column, _QF) :-
    ...
    write(Column),
    nl,
    append_to_output_nl(['**create_one_python_entry/2 bad column parameter in schmema']),
    halt.
...
create_SPARQL_select_statement(_Subject, _Columns, QF) :-
    write(QF, 'SELECT'),
    % Save a lot of faffing by not listing the variable names in the query
    % * returns all variables anyway
    write(QF, ' *'),
...
create_SPARQL_select_subject(Subject, _QF) :-
    write('**create_SPARQL_select_subject/2 bad subject paramater'),
    write(Subject),
    nl,
    append_to_output_nl(['**create_SPARQL_select_subject/2 bad subject paramater']),
    halt.
...
create_SPARQL_select_list([Column|_Columns], _QF) :-
    ...
    write(Column),
    nl,
    append_to_output_nl(['**create_SPARQL_select_list/2 bad column paramater in schema ', Column]),
    halt.
...
create_SPARQL_where_subject(subject, QF) :-
    !,
    write(QF, '?subject ').
% Or if we've given the subject a (fully realised) value, then match on the value.
...
create_SPARQL_where_subject(Subject, _QF) :-
    ...
    write(Subject),
    nl,
    append_to_output_nl(['**create_SPARQL_where_subject/2 bad subject paramater']),    
    halt.
...
write_SPARQL_value(_QF, Value) :-
    ...
    append_to_output_nl(['**write_SPARQL_value ERROR term must not contain Prolog variables']),
    halt.
write_SPARQL_value(QF, Value) :-
...
write_SPARQL_value(QF, Value) :-
    atom(Value),  % An other atom
    utils:associatedPrefix(Value, Prefix),  % it has a prefix ...
    !,
    atom_concat(Prefix, Value, Uri),  % so treat it as a URI
    % write the value as a URI
    write(QF, '<'), write(QF, Uri),  write(QF, '>').
%% write_SPARQL_value(QF, Value) :-
...
write_SPARQL_value(QF, Value) :-
    atom(Value),  % Any other atom
    !,
    % Assume it's a Resource URI in the DBpedia ontology
    atom_concat('http://dbpedia.org/resource/', Value, Uri),  % so treat it as a URI
    % write the value as a URI
    write(QF, '<'), write(QF, Uri),  write(QF, '>').
...
write_SPARQL_value(_QF, Value) :-
    ...
    write(Value), nl,
    append_to_output_nl(['**write_SPARQL_value ERROR unregnised SPARQL value in query: ', Value]),
    halt.
...
create_SPARQL_where_columns([], _QF) :-
    ...
    nl,
    append_to_output_nl(['**create_SPARQL_where_columns/2 No columns in schema']),
    halt.
create_SPARQL_where_columns(_Columns, _QF) :-
    write('**create_SPARQL_where_columns/2 Bad columns in schema'),
    nl,
    append_to_output_nl(['**create_SPARQL_where_columns/2 Bad columns in schema']),
    halt.
...
create_SPARQL_where_column_sub(Column, _QF) :-
    ...
    write(Column),
    nl,
    append_to_output_nl(['**create_SPARQL_where_column_sub/2 bad column parameter ']),
    halt.
...
perform_query(n3, _Dataset, Result) :-
    ...
    environment_variable('TARGET_URI', Target),
%     sparql_output_mode(table, FileName, SparqlDirection, Size), %% csv output
    ...
    atoms_concat(['rsparql', 
		  ' --query ', SparqlQuery,
		  ' --service ', Target,
		  SparqlDirection, SparqlQueryResults], Query),  %% Table format
    nl,
    write('Execute query on service '), write(Target), nl, nl,
...
executed_query(QueryResults, Size, success) :-
    ...
    !,
    nl, write('The query succeeded. The returned answers are:'), nl,
...
executed_query(_QueryResults, _Size, no_data) :-
    nl, write('The query did not return any data.'), nl,
    append_to_output_nl(['The query did not return any data.']).
